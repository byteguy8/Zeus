...
static ObjHeader *sieve_eratosthenes(const int64_t until, void *context){
    aidx_t len = until - 1;
    int64_t numbers[len];
    ...
    ObjHeader *array_obj = vmu_create_array_obj(primes_len, context);
...
Value native_fn_primes(uint8_t argsc, Value *values, Value *target, void *context){
    Value *until_value = &values[0];

    VALIDATE_VALUE_INT_ARG(until_value, 1, "until", context)

    int64_t until = VALUE_TO_INT(until_value);

    VALIDATE_VALUE_INT_RANGE_ARG(until_value, 0, "until", 2, ARRAY_LENGTH_MAX, context)

    ObjHeader *array_obj = sieve_eratosthenes(until, context);
...
Value native_fn_sqrt(uint8_t argsc, Value *values, Value *target, void *context){
    ...
    if(!IS_VALUE_FLOAT(raw_value)){
        vmu_error(context, "Expect float, but got something else");
    }
...
Value native_fn_pow(uint8_t argsc, Value *values, Value *target, void *context){
    ...
    if(!IS_VALUE_FLOAT(base_value)){
        vmu_error(context, "Expect float at argument 0 (base), but got something else");
    }
    if(!IS_VALUE_FLOAT(base_value)){
        vmu_error(context, "Expect float at argument 1 (exponent), but got something else");
    }
