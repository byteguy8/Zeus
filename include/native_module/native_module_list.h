...
Value native_fn_list_add(uint8_t argsc, Value *values, Value *target, void *context){
...
Value native_fn_list_remove(uint8_t argsc, Value *values, Value *target, void *context){
    ...
    return vmu_list_remove(at, target_list_obj, VMU_VM);
}
...
NativeFn *native_list_get(size_t key_size, char *key, VM *vm){
    if(!list_symbols){
        ...
        factory_add_native_fn_info_n("add", 1, native_fn_list_add, list_symbols, allocator);
