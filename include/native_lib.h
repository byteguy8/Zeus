...
static void *to_str(void *raw){
    Value *value = (Value *)raw;
    return value->literal.obj;
}

static void *to_array(void *raw){
    Value *value = (Value *)raw;
    return value->literal.obj;
}
...
static Value *str_value(void *raw){
    memset(&temp_value, 0, sizeof(Value));
    temp_value = OBJ_VALUE((Obj *)raw);
    return &temp_value;
}

static Value *array_value(void *raw){
    memset(&temp_value, 0, sizeof(Value));
    temp_value = OBJ_VALUE((Obj *)raw);
    return &temp_value;
}
...
void native_lib_init(void *handler, VM *v){
    ...
    znative_set_to_str(to_str);
    znative_set_to_array(to_array);

    znative_set_empty_value(empty_value);
    znative_set_bool_value(bool_value);
    znative_set_int_value(int_value);
    znative_set_str_value(str_value);
    znative_set_array_value(array_value);
