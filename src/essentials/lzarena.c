...
void *lzarena_save(LZArena *arena){
    LZRegion *current = arena->current;
    void *current_block_offset = current ? current->block_offset : NULL;
    LZArenaState *state = LZARENA_ALLOC(arena, sizeof(LZArenaState));
...
int lzarena_restore(LZArena *arena, void *raw_state){
	LZArenaState state = *(LZArenaState *)raw_state;
    ...
    if(state.region){
        arena->last_restored = state.id;
        arena->used_memory = state.used_memory;
        arena->current = state.region;
        arena->current->block_offset = state.block_offset;
    }
...
inline void lzarena_reset_states(LZArena *arena){
    arena->last_restored = 0;
    arena->states_counter = 0;
}
