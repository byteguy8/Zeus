...
static int grow(char multiplier, size_t required, LZBStr *str){
    ...
    void *new_buff = MEMORY_REALLOC(str->buff, char, buff_len, new_buff_len, str->allocator);
...
inline size_t lzbstr_available_space(LZBStr *str){
    size_t available = str->buff_len - str->offset;
    return available == 0 ? 0 : available - 1;
}
